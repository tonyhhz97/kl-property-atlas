...
table{width:auto;max-width:100%;border-collapse:collapse;font-size:1.045rem}
...
let state = {
  ...
  budgetRanges:[],
  rooms:[], pet:"Any", completion:[], title:"Any",
...
function getFiltered(){
  let list = PROJECTS.filter(p=>{
    ...
    // Budget + Rooms: only applied once at least one is selected. Checked
    // together against the SAME unit, not as two independent "some unit
    // matches budget" / "some other unit matches rooms" checks - otherwise
    // a project could pass both filters purely because one cheap studio
    // happens to be in budget while a totally different, pricier 3-room
    // unit satisfies the room count, even though no single unit actually
    // meets both picks at once. A client who picks "RM500k-700k" + "3 rooms"
    // wants a unit that is BOTH within that price range AND has 3 rooms.
    ...
    if(state.budgetRanges.length || state.rooms.length){
...
function resetFilters(){
  state = {...state, search:'', areas:[], tenure:'All', budgetRanges:[], rooms:[], pet:'Any', completion:[], title:'Any'};
  ...
  document.getElementById('applyFiltersBtn')?.classList.remove('pending');
  renderAreaTiles(); renderRoomTicks(); renderCompletionTicks(); renderBudgetTicks(); renderTable();
}
...
function jumpToProject(name){
  ...
  document.getElementById('titleSelect').value='Any';
  document.getElementById('applyFiltersBtn')?.classList.remove('pending');
  renderAreaTiles(); renderRoomTicks(); renderCompletionTicks(); renderBudgetTicks();
  expandAndScrollTo(name);
}
